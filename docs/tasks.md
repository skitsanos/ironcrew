...
| `${results.task_name.token_usage.prompt_tokens}`     | Prompt tokens consumed by the task       |
| `${results.task_name.token_usage.completion_tokens}` | Completion tokens generated by the task  |
| `${results.task_name.token_usage.total_tokens}`      | Total tokens (prompt + completion)       |
| `${results.task_name.token_usage.cached_tokens}`     | Cached prompt tokens (when the provider reports them) |
