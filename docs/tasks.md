...
> For nested sub-crew invocation from within a task's agent tools (or from
> `crew.lua` itself), see [`run_flow()`](tools.md#run_flow-sub-crew-delegation).
...
| `${results.task_name.usage.settled.prompt_tokens.known}` | Known prompt-token subtotal (decimal string or null) |
| `${results.task_name.usage.settled.completion_tokens.known}` | Known completion-token subtotal |
| `${results.task_name.usage.settled.total_tokens.known}` | Known total-token subtotal |
| `${results.task_name.usage.coverage}` | `complete`, `partial` or `unavailable`; inspect field completeness too |
...
requires the full response to process tool invocations. This applies to **any**
tool the agent can invoke — built-in tools, custom Lua tools, and MCP client
tools (names starting with `mcp__`).
