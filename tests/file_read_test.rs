...
async fn test_file_write_and_read() {
    ...
    write_tool.execute(json!({"path": "output.txt", "content": "test content"})).await.unwrap();

    let read_tool = FileReadTool::new(Some(dir.path().to_path_buf()));
    let result = read_tool.execute(json!({"path": "output.txt"})).await.unwrap();
...
async fn test_file_write_blocked_extension() {
    ...
    let result = tool.execute(json!({"path": "evil.exe", "content": "bad"})).await;
    assert!(result.is_err());
