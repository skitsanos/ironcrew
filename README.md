...
- **Conversation mode** — stateful multi-turn chat with an agent (`crew:conversation({})`), with tool support, streaming, and reasoning capture
...
**Features:** `simple` · `research-crew` · `json-output` · `parallel` · `collaborative` · `memory` · `foreach` · `streaming` · `subworkflow` · `model-router` · `conditional-crew` · `http-api` · `batch-processing` · `config-lua` · `conversation`
